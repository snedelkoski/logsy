...
class PCA(object):
    ...
    def evaluate(self, X, y_true):
        ...
        acc, precision, recall, f1 = metrics(y_pred, y_true)
        print('Accuracy: {:.3f}, Precision: {:.3f}, recall: {:.3f}, F1-measure: {:.3f}\n'
              .format(acc, precision, recall, f1))
        return acc, precision, recall, f1

    def evaluate_scores(self, scores, y_true):
        print('====== Evaluation summary ======')
        y_pred = self.run_threshold(scores)
        acc, precision, recall, f1 = metrics(y_pred, y_true)
        print('Accuracy: {:.3f}, precision: {:.3f}, recall: {:.3f}, F1-measure: {:.3f}\n'
              .format(acc, precision, recall, f1))
        return acc, precision, recall, f1
